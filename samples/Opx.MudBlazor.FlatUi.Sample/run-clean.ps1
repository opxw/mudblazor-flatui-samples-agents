...
foreach ($directoryName in @("bin", "obj")) {
    $target = [System.IO.Path]::GetFullPath((Join-Path $projectRoot $directoryName))
    $expected = "$projectRoot$([System.IO.Path]::DirectorySeparatorChar)$directoryName"
...
Write-Output "Cleaned project-local bin and obj directories."
$nugetConfig = $null
