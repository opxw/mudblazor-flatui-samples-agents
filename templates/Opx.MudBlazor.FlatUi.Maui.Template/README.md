...
It uses NuGet `Opx.MudBlazor.FlatUi` 2.1.29, restores Device/Manual font size and density before first paint, and configures package-owned panel headers with `PanelHeaderMinHeight=48` and `PanelHeaderPaddingY=10`. Preserve those public options; do not replace them with consumer CSS overrides.
