...
1. Mount `RootStartupGate` directly from the root app and start in `Checking`, before `Router` is created—even for an anonymous requested route. Initialize display preferences and resolve Light/Dark/Auto plus palette before host authorization validation. Render only full-page `FlatSessionRestore`, with copy sourced from typed `FlatReconnectOptions.StartupTitle`/`StartupMessage`, inside a loading-only `FlatMudProviders` branch bound to that resolved theme/palette so MudBlazor services, package variables, and spinner visual are active. After validation, remove that branch and create `Router`; the selected layout owns its normal provider, so providers are never nested. Do not create, prerender, or briefly reveal `MainLayout` or protected route content. In MAUI use `Title="Memuat"` and keep its package-owned `.flat-system-loading-spinner` visually continuous with the preceding static `BlazorWebView` bootstrap placeholder and with `FlatReconnectModal` Reconnecting. Text-only, blank, square/un-themed, or duplicate loading states are invalid.
