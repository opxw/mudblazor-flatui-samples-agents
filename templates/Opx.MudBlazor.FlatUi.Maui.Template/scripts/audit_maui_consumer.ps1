...
Require-Text $project[0].FullName @(
    ...
    'Opx.MudBlazor.FlatUi" Version="2.1.26',
...
if (Test-Path -LiteralPath $assetsPath) {
    try {
        ...
        $packageCss = Join-Path $packageRoot "opx.mudblazor.flatui\2.1.26\staticwebassets\opx-flat-ui.css"
        if (-not (Test-Path -LiteralPath $packageCss)) { $violations.Add("Restored OPX package CSS was not found.") }
        elseif ((Get-FileHash -LiteralPath $packageCss -Algorithm SHA256).Hash -ne "195B0111B3F1F8592034C1B123DC8B977607FA293F49A02D1A034EAFAC483FA3") { $violations.Add("Restored OPX 2.1.26 CSS hash does not match the contract.") }
...
Write-Output "OPX Flat UI MAUI consumer audit passed for '$($project[0].FullName)' (contract 2.1.26$(if ($ExactSample) { ', Exact Sample Mode' }))."
