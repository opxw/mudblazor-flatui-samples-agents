...
Require-Text $project[0].FullName @(
    ...
    'Opx.MudBlazor.FlatUi" Version="2.1.25',
...
if (Test-Path -LiteralPath $assetsPath) {
    try {
        ...
        $packageCss = Join-Path $packageRoot "opx.mudblazor.flatui\2.1.25\staticwebassets\opx-flat-ui.css"
        if (-not (Test-Path -LiteralPath $packageCss)) { $violations.Add("Restored OPX package CSS was not found.") }
        elseif ((Get-FileHash -LiteralPath $packageCss -Algorithm SHA256).Hash -ne "43283F075176C584A8AE7EC4D60846CF8BFA4959A09384082D48CE206474BDD2") { $violations.Add("Restored OPX 2.1.25 CSS hash does not match the contract.") }
...
Write-Output "OPX Flat UI MAUI consumer audit passed for '$($project[0].FullName)' (contract 2.1.25$(if ($ExactSample) { ', Exact Sample Mode' }))."
