...
Its `flat-ui.mobile.contract.json` pins MAUI `10.0.90`, CommunityToolkit.Maui `15.0.1`, MudBlazor `9.9.0`, and OPX Flat UI `2.1.12`. Android compilation is required source evidence; iOS build/runtime and all native UX claims still require macOS/Xcode plus simulator/device evidence.
...
- Reusable OPX styling comes only from the restored NuGet static web asset `_content/Opx.MudBlazor.FlatUi/opx-flat-ui.css`. Contract `2.1.12` pins its SHA-256 so a stale or substituted package asset fails audit.
...
- Responsive card lists own vertical scrolling only while their content overflows. In 2.1.12, vertical pan chains to the page at a non-overflowing list or list boundary, while horizontal overscroll remains contained; consumers must not restore the older trapped-scroll behavior with local CSS.
- AppBar global search is visible by default and may be disabled with `OpxFlatUi:Display:AppBarSearchVisible=false`. The host binds that value to `FlatAppShell.SearchVisible`; disabled means no search markup, popup, reserved width, or focus target.
- The consumer starts with Light theme and Device/browser font ownership (`UseAppFontSize=false`). Settings always exposes Device/Manual; Manual uses the bounded `16px` default, while Device follows browser `1rem` and preserves accessibility scaling. This intentionally differs from the source showroom's theme default while retaining its `2.1.12` font behavior.
