...
- A destination has zero permission to override reusable CSS. Local stylesheets must not target package/MudBlazor selectors such as `.flat-*` or `.mud-*`, redeclare `--opx-*` tokens, copy package rules, or use specificity/`!important` to repair canonical geometry. Configure supported appearance through typed options, component parameters, documented public seams, and Settings. Destination CSS is limited to newly named application/domain composition classes that neither target package internals nor alter canonical geometry, spacing, responsive behavior, or states.
...
- Record every intentional destination deviation. A deviation is allowed only for a stated business requirement and explicit decision, but approval does not authorize a package CSS override in the destination. A reusable visual change must be implemented and released by the package/source; missing canonical markup/classes, local package CSS overrides, or accidental spacing/responsive drift is `bug integrasi aplikasi`.
